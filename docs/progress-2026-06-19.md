...
- 完成 v0.2 测试与稳定性第一批。
  - 新增后端 pytest 测试：
    - TXT 上传后自动解析到 `ready`
    - GB18030 TXT 上传解析
    - 阅读进度 GET/PUT 和非法句子校验
    - 删除书籍清理数据库记录、阅读进度、任务、源文件和音频文件
  - 新增 `scripts/smoke_api.py`，用于服务启动后的 API smoke 验收
  - 修正 `backend/pyproject.toml` dev 依赖：`httpx2` -> `httpx`
  - 将 `Job.payload` 改为跨数据库兼容的 JSON 类型，PostgreSQL 下仍使用 JSONB
  - 修复解析 worker 从 job payload 读取 `book_id` 时未显式转换 UUID 的问题
  - 应用启动时自动确保 `storage/uploads`、`storage/parsed`、`storage/audio` 目录存在
  - 时间戳默认值改为 timezone-aware UTC
  - 重写 `README.md` 和 `docs/runbook.md`，补充自动化验证和手工浏览器验收流程
